                ...
                if (details.settlements.isNotEmpty()) {
                    ...
                    details.settlements.forEach { settlement ->
                        Column(Modifier.fillMaxWidth()) {
                            ...
                            Text("${formatMoney(settlement.amountRial)} · ${settlement.paymentMethod}")
...
private fun SettlementDialog(
    ...
    var amount by remember(details.paidRial) { mutableStateOf(details.outstandingRial.toString()) }
...
private fun purchaseStatusTitle(status: String): String = when (status) {
    "PAID" -> "تسویه‌شده"
    "PARTIAL" -> "پرداخت ناقص"
